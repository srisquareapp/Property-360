...
function landForm(){shell("1-B & Adangal",`<div class="panel">${field("landDistrict","District","",currentProperty?.district||"")}${field("landMandal","Mandal","",currentProperty?.mandal||"")}${field("landVillage","Village","",currentProperty?.village||"")}${field("landSurvey","Survey Number","",currentProperty?.survey||"")}<button class="btn" onclick="portal('meebhoomi')">Open Official MeeBhoomi</button><button class="btn secondary" onclick="openMeeBhoomiNewTab()">Open MeeBhoomi in New Tab</button><div class="note">MeeBhoomi is the official government portal. If Safari returns 403 while Chrome works, use the Chrome fallback. Property 360 does not bypass CAPTCHA, OTP or other security controls.</div></div>`)}
